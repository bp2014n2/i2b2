...
frequence_of_visits <- function(patientsWithObservation, patientsWithoutObservation) {
  ...
  text(head(p.with.agg$date, 1),max(p.without.agg$p.without.data, p.with.agg$p.with.data)*0.9,"Mit Rückenschmerzen",adj=0,cex=1,col=farbe1_150)
  text(head(p.with.agg$date, 1),max(p.without.agg$p.without.data, p.with.agg$p.with.data)*0.85,"Ohne Rückenschmerzen",adj=0,cex=1,col=farbe2_150)
  ...
  if(!is.null(input.params$icd)) {
     mtext(paste0("Analyzed ICD: ", input.params$icd),3,line=0,adj=0,cex=0.9,outer=t)
  } else {
     mtext("ICD:54",3,line=0,adj=0,cex=0.9,outer=t)
  }
