...
import de.erlangen.i2b2.giri.datavo.pdo.ConceptSet;
import de.erlangen.i2b2.giri.datavo.pdo.EventSet;
import de.erlangen.i2b2.giri.datavo.pdo.ModifierSet;
import de.erlangen.i2b2.giri.datavo.pdo.ObservationSet;
import de.erlangen.i2b2.giri.datavo.pdo.ObserverSet;
import de.erlangen.i2b2.giri.datavo.pdo.PatientSet;
import de.erlangen.i2b2.giri.datavo.pdo.query.ItemType;
import de.erlangen.i2b2.giri.datavo.pdo.query.PatientDataResponseType;
import de.erlangen.i2b2.giri.datavo.pdo.query.PdoQryHeaderType;
import de.erlangen.i2b2.giri.datavo.pdo.query.RequestType;
import de.erlangen.i2b2.giri.datavo.giriconfig.InputType;
import de.erlangen.i2b2.giri.datavo.giriconfig.OutputType;
import de.erlangen.i2b2.giri.datavo.giriconfig.RscriptletType;
import de.erlangen.i2b2.giri.datavo.girimessages.AdditionalInputType;
import de.erlangen.i2b2.giri.datavo.girimessages.PatientSetsType;
import de.erlangen.i2b2.giri.datavo.girimessages.ConceptsType;
import de.erlangen.i2b2.giri.datavo.girimessages.RScriptletResultType;
import de.erlangen.i2b2.giri.datavo.girimessages.RResultsType;
import de.erlangen.i2b2.giri.datavo.girimessages.ResultType;
...
public class GetRResultsRequestHandler implements RequestHandler {
	...
	public String handleRequest(RequestMessageType input) throws I2B2Exception {
		...
		try {
			...
			log.error("Incoming request XML is not valid. Stack trace: " + GIRIUtil.getStackTraceAsString(e));
		...
		RScriptletResultType giriResType = null;
		String scriptletDirectoryName = null;
		String QTSUrl = null;
		PatientSetsType patSetType = null;
		ConceptsType conceptsType = null; 
		try {
			giriResType = (RScriptletResultType) unwrapHelper.getObjectByClass(input.getMessageBody().getAny(), RScriptletResultType.class);
			scriptletDirectoryName = giriResType.getRScriptletName();
			QTSUrl = giriResType.getQTSUrl();
			patSetType = giriResType.getPatientSets();
			conceptsType = giriResType.getConcepts();
			if (scriptletDirectoryName == null || QTSUrl == null || patSetType == null || conceptsType == null) {
				throw new NullPointerException();
			}
		} catch (Exception e) {
			log.error("Incoming XML request body is not valid or complete. Stack trace: " + GIRIUtil.getStackTraceAsString(e));
			throw new I2B2Exception("Error message delivered from server: Incoming XML request body is not valid or complete.");
		}
		AdditionalInputType addInType = giriResType.getAdditionalInput();
		
		// Assemble scriptlet path
		scriptletDirectoryName.replace("/", ""); // for security
		scriptletDirectoryName.replace("..", ""); // for security
		scriptletDirectoryName.replace("\\", ""); // for security
		String scriptletDirectoryPath = GIRIUtil.getRSCRIPTLETPATH() + "/" + scriptletDirectoryName;
		...
		try {
			giriType = GIRIUtil.validateAndUnmarshallScriptletConfigFile(scriptletDirectoryPath, scriptletDirectoryName);
		...
		if (giriType == null) {
			log.error("Error during config file validation (giriType==null)");
			throw new I2B2Exception("Error delivered from server: Validation error (giriTaype==null)");
		}
		
		// Extract additional input values if there are any
		Map<String,String> inputParametersMap = new HashMap<String, String>();
		if (giriType.getAdditionalInputs() != null) {
			// For handy access convert the request input parameter list into a map
			Map<String, String> inputParametersFromRequest = null;
			if (addInType != null) {
				inputParametersFromRequest = GIRIUtil.convertListIntoMapAndDecodeHTML(addInType.getInputParameter());
			}
			// For every specified (in config file!) additional input value...
			for (InputType ipt : giriType.getAdditionalInputs().getInput()) {
		...
		if (giriType.getCustomOutputs() != null) {
			for (OutputType ot : giriType.getCustomOutputs().getOutput()) {
		...
		String extendedWebdirPath = GIRIUtil.getWEBDIRPATH() + "/userfiles/" + username + "/";
		...
		for (Integer pst  : patSetType.getPatientSetCollId()) {
			...
			de.erlangen.i2b2.giri.datavo.pdo.query.ObjectFactory pdoQryFac = new de.erlangen.i2b2.giri.datavo.pdo.query.ObjectFactory();
			...
			JRIProcessor.readDataFrameFromString("GIRI.patients[[" + i + "]]", CRCResponseParser.parsePatientSet(crcPS), CRCResponseParser.patientsColClasses);
			if (conceptAvailable) {
				JRIProcessor.readDataFrameFromString("GIRI.observations[[" + i + "]]", CRCResponseParser.parseObservationSet(crcOS, crcCS), CRCResponseParser.conceptsColClasses);
				JRIProcessor.readDataFrameFromString("GIRI.events[[" + i + "]]", CRCResponseParser.parseEventSet(crcES), CRCResponseParser.eventsColClasses);
				JRIProcessor.readDataFrameFromString("GIRI.modifiers[[" + i + "]]", CRCResponseParser.parseModifierSet(crcMS), CRCResponseParser.modifierColClasses);
				JRIProcessor.readDataFrameFromString("GIRI.observers[[" + i + "]]", CRCResponseParser.parseObserverSet(crcObS), CRCResponseParser.observersColClasses);
			}
			i++;
		}
		...
		if (giriType.getSettings().isPassROutput() == null || giriType.getSettings().isPassROutput()) {
		...
		if (giriType.getSettings().isPassRErrors() == null || giriType.getSettings().isPassRErrors()) {
		...
		bodType.getAny().add(girimesFac.createRResults(rrt));
