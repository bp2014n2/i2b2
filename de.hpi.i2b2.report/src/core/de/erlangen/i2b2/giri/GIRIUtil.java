...
import de.erlangen.i2b2.giri.datavo.girimessages.InputParameterType;

import edu.harvard.i2b2.common.exception.I2B2Exception;
import edu.harvard.i2b2.common.util.jaxb.JAXBUtil;

// Helper class with some useful methods and constants
public class GIRIUtil {
	
	// Needed for jaxb initialization
    private static final String[] DEFAULT_PACKAGE_NAME = new String[] {
        "de.erlangen.i2b2.giri.datavo.i2b2message",
        "de.erlangen.i2b2.giri.datavo.pdo",
        "de.erlangen.i2b2.giri.datavo.pdo.query",
        "de.erlangen.i2b2.giri.datavo.giriconfig",
        "de.erlangen.i2b2.giri.datavo.girimessages"
    };
    private static Log log = LogFactory.getLog(GIRIUtil.class);
    ...
    // testingmode is for local testing without binding aar/jar packages and deploying it to jboss. Just call a service method in GIRIService
	...
	public static void initializeGIRIUtil() {
		...
		if (testingmode) {
			...
			try {
				resources = GIRIUtil.class.getClassLoader().getResources("etc/build.properties");
				prop.load(resources.nextElement().openStream());
				RSCRIPTLETPATH = prop.getProperty("GIRI.directory");
	...
	public static URL getCONFIGSCHEMAURL() throws I2B2Exception {
		if (testingmode) {
			...
			// Schema file GIRIConfig.xsd is packed into the archive file GIRI.aar and is accessed by the following code
			Enumeration<URL> resources = null;
			try {
				resources = GIRIUtil.class.getClassLoader().getResources("etc/GIRIConfig.xsd");
			} catch (IOException e) {
	...
	public static RscriptletType validateAndUnmarshallScriptletConfigFile(String scriptletPath, String scriptletName) throws I2B2Exception, SAXException {
		...
		if (scriptletconfig.exists()) {
			...
			try {
				giriType = (RscriptletType) getJAXBUtil().unMashallerRequest(scriptletconfigpath).getValue();
				if (giriType == null) throw new NullPointerException();
			} catch (Exception e) {
				throw new I2B2Exception("Error during unmarshalling a scriptlet config file", e);
			}
			
		} else {
			// If no config file is provided, just create a minimal GIRI object
			de.erlangen.i2b2.giri.datavo.giriconfig.ObjectFactory giriconfFac = new de.erlangen.i2b2.giri.datavo.giriconfig.ObjectFactory();
			giriType = giriconfFac.createRscriptletType();
			giriType.setSettings(giriconfFac.createSettingsType());
		}
