...
public class JRIProcessor {
    ...
    public static File prepare(String webDirPath) throws I2B2Exception {
    	...
    	REXP ret2 = re.eval("report.patients <- c()");
    	REXP ret3 = re.eval("report.observations <- c()");
    	REXP ret4 = re.eval("report.input <- c()");
    	REXP ret5 = re.eval("report.output <- list()");
    	REXP ret6 = re.eval("report.concept.names <- c()");
    	REXP ret7 = re.eval("report.modifiers <- c()");
    	REXP ret8 = re.eval("report.events <- c()");
    	REXP ret9 = re.eval("report.observers <- c()");
    	...
    	re.eval("report.swapPlusMinus <- function(x) if (!is.na(x)){if(x==\"-\") {\"+\"} else {\"-\"}}");
    	re.eval("setAs(\"character\",\"i2b2DateTime\", function(from){do.call(c,lapply(from, function(x) {as.POSIXlt(x, tz = paste(\"GMT\", report.swapPlusMinus(substr(x,24,24)), substr(x,26,26), sep=\"\"), format=\"%Y-%m-%dT%H:%M:%S\")}))})");
    	return f;
    }
    ...
    public static void assignAdditionalInput(Map<String, String> m) throws I2B2Exception {
    	...
    	for (Map.Entry<String, String> entry : m.entrySet()) {
    		...
    		value = value.replace("\"", "\\\"");
    		REXP ret = re.eval("report.input[\"" + key + "\"] = \"" + value + "\"");
    		if (ret == null) {
    ...
    public static void assignConceptNames(String[] names) {
    	for (int i = 0; i < names.length; i++) {
        	...
        	re.eval("report.concept.names[" + (i+1) + "] <- \"" + sanitized + "\"");
    	}
    }
    ...
    public static List<String[]> getOutputVariables(List<String[]> outputParametersList, String webPath) throws I2B2Exception {
    	...
    	REXP ret = re.eval("report.output.1");
    	for (int i = 2; ret != null; i++) {
    		String name = "report.output." + (i-1); // Default name
    	...
    	for (String[] oElement : outputParametersList) {
    		...
    		oName = oName.replace("\"", "\\\"");
    		String Rname = "report.output[[\"" + oName + "\"]]"; // Name to access output variable in R
    ...
    private static String extractResult(String type, String name, String csvPath, String filename) throws I2B2Exception{
    	if (type.equals("data.frame") || type.equals("matrix")) {
    		...
    		REXP transformRet = re.eval("report.tmptable <- as.data.frame(lapply(" + name + ", function(x) if (is(x, \"POSIXt\")) format(x, \"%Y-%m-%d %H:%M:%S\") else x))");
    		REXP csvRet = re.eval("write.table(report.tmptable, file = \"" + csvPath + "/" + filename + ".csv\", append = FALSE, quote=which(sapply(" + name + ", function(x) !is.numeric(x) & !is(x, \"POSIXt\")))," +
    				" sep = \",\", eol = \"\\r\\n\", na = \"NULL\", dec = \".\", row.names = FALSE, col.names = TRUE, qmethod=\"double\", fileEncoding = \"UTF-8\")");
    		REXP rmTab = re.eval("rm(report.tmptable)");
...
class ScriptExecutorCallbackClass implements RMainLoopCallbacks {	
	...
	public void rWriteConsole(Rengine re, String text, int oType) {
		...
		// Error output
			...
			// Do not send back the error 'Error: object 'report.output.x' not found' because this 'error' appears every time
			...
			if ( !(text.contains("Error: object 'report.output.") && text.contains("not found")) ) {
