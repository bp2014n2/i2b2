...
if (length(report.concept.names) < 2) {
	write("Script error: Please specify two concepts!\n", stderr())
}

# Initialize data structures
patient.event.dates <- list()
risk.concept <- report.concept.names[1]
target.concept <- report.concept.names[2]
km.id <- c()
km.period <- c()
km.status <- c()
km.group <- c()
...
for (i in 1:length(report.observations)) {
	
	# Error case: No observations / empty patient set
	if (dim(report.observations[[i]])[1] == 0) {
	...
	for (j in 1:dim(report.observations[[i]])[1]) {
		
		# Some abbreviations
		tmp.concept.path <- report.observations[[i]][j,"concept_path"]
		tmp.patient.id <- report.observations[[i]][j,"patient_id_value"]
		tmp.start.date <- report.observations[[i]][j,"start_date"]
...
for (k in 1:length(report.observations)) {
...
for (m in 1:length(report.observations)) {
...
# Plot it
if (report.input["Colorful or black-white plot graphic?"] == "Colorful") {
