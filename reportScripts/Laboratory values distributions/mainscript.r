# Abbrevations
binsNumber <- as.numeric(report.input["Number of bins"])
groupname1 <- report.input["Name of patient set 1"]
groupname2 <- report.input["Name of patient set 2"]
mainname <- report.input["Plot label"]
vals1 <- report.observations[[1]]$nvalnum_value[!is.na(report.observations[[1]]$nvalnum_value)]
vals2 <- report.observations[[2]]$nvalnum_value[!is.na(report.observations[[2]]$nvalnum_value)]
